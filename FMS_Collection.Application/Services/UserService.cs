...
namespace FMS_Collection.Application.Services
{
    public class UserService
    {
        ...
        public UserService(IUserRepository repository, OtpService otpService, AzureBlobService blobService)
        {
        ...
        public async Task<ServiceResponse<LoginResponse>> GetLoginDetails(LoginRequest user)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(loginResponse.ImagePath))
                {
                    loginResponse.ImagePathSasUrl = _blobService.GetBlobSasUrl(loginResponse.ImagePath);
                }

                if (!string.IsNullOrEmpty(loginResponse.ThumbnailPath))
                {
                    loginResponse.ThumbnailPathSasUrl = _blobService.GetBlobSasUrl(loginResponse.ThumbnailPath);
                }
