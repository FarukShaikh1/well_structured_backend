...
public class AzureBlobService
{
    ...
    public AzureBlobService(IConfiguration configuration)
    {
        ...
        {
            connectionString = "DefaultEndpointsProtocol=https;AccountName=fmsprojectfiles;AccountKey=kVBAec2AQyS9AF4fZzWeWyv0uOTlUrcjVj9jzf2AIaruUAUvulRNDpeUD3sG5j8+PR5wc/ML7KVo+AStrD+Bbg==;EndpointSuffix=core.windows.net";
        }
        if (String.IsNullOrEmpty(_containerName))
        {
            _containerName = "project-attachments";
        }
    ...
    public string GetBlobSasUrl(string blobName, int validHours = 24)
    {
        ...
        var sasBuilder = new BlobSasBuilder
        {
            ...
            BlobName = blobName,
            Resource = "b",
            ExpiresOn = DateTimeOffset.Now.AddHours(validHours) // Valid for 24 hours
        };
    ...
    public async Task UploadFileAsync(byte[] fileBytes, string blobPath, string contentType)
    {
        var containerClient = _blobServiceClient.GetBlobContainerClient(_containerName);
        ...
        {
            ContentType = contentType, // or detect dynamically
    ...
    public async Task<string> GetDownloadSasUrl(string blobPath, string fileName)
    {
        var containerClient = _blobServiceClient.GetBlobContainerClient(_containerName);
        var blobClient = containerClient.GetBlobClient(blobPath);
        ...
        BlobSasBuilder sasBuilder = new BlobSasBuilder
        {
            ...
            BlobName = blobPath,
            Resource = "b",
            StartsOn = DateTimeOffset.Now.AddMinutes(-5),
    ...
    public async Task UpdateBlobHeadersInFolderAsync(string folderPrefix)
    {
        ...
        await foreach (var blobItem in containerClient.GetBlobsAsync(prefix: folderPrefix))
        {
            ...
            var headers = new BlobHttpHeaders
            {
                ...
                ContentDisposition = "inline" // ✔ default inline preview
            };
