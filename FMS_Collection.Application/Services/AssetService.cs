...
namespace FMS_Collection.Application.Services
{
    public class AssetService
    {
        ...
        public async Task<ServiceResponse<List<Assets>>> GetAllAssetsAsync()
        {
        ...
        public async Task<Guid?> UpdateAssetAsync(AssetRequest Asset, Guid userId)
        {
            return await _repository.UpdateAsync(Asset, userId);
        }
        ...
        public async Task<ServiceResponse<Guid?>> UpdateFile(IFormFile file, Guid userId, Guid? assetId, string folder = "Other")
        {
            var old = await _repository.GetAssetDetailsAsync(assetId);
            await DeleteOldDocStoreAssetFiles(old);
            var stored = await UploadDocumentToDocStore(file, folder, file.FileName);
            ...
            serviceResponse.Data = assetId;
            serviceResponse.Message = Messages.AssetSavedSuccessfully;
        ...
        public async Task<Guid?> UploadDocument(DocumentRequest document, Guid userId)
        {
            ...
            var request = CreateAssetRequest(stored, document.file, document.AssetId);
            if (!document.AssetId.HasValue || document.AssetId.Value != Guid.Empty)
            {
        ...
        public async Task<int> CopyFiles(string sourcePath)
        {
            ...
            try
            {
                ...
                List<Assets> files = await _repository.GetAllAsync();
        ...
        private async Task<AssetResponse> UploadDocumentToDocStore(IFormFile file, string folder, string fileName)
        {
            ...
            string uniqueName = $"{Path.GetFileNameWithoutExtension(fileName)}_{DateTime.Now:yyyyMMddHHmmssfff}{extension}";
            var fileBytes = await GetBytes(file);
            // Detect MIME correctly
            string contentType = file.ContentType ?? "application/octet-stream";

            string originalPath = await UploadFileAsync(folder, uniqueName, fileBytes, contentType);

            string category = GetFileCategory(file.ContentType, file.FileName);

            string? thumbnailPath = await GenerateThumbnail(category, fileBytes, folder, uniqueName, contentType);
        ...
        private async Task<string> UploadFileAsync(string folder, string fileName, byte[] fileBytes, string contentType)
        {
            //string uniqueFileName = Guid.NewGuid().ToString() + Path.GetExtension(fileName);
            string blobPath = $"{folder}/{fileName}";
            await _blobService.UploadFileAsync(fileBytes, blobPath, contentType);
            return blobPath;
        }

        private async Task<string> UploadThumbnailAsync(string folder, string uniqueFileName, byte[] fileBytes, string contentType)
        {
            string thumbFileName = $"thumb_{uniqueFileName}.webp"; // Save as WebP
            ...
            await _blobService.UploadFileAsync(outStream.ToArray(), thumbPath, contentType);
        ...
        private AssetRequest CreateAssetRequest(AssetResponse storedFile, IFormFile file, Guid? assetId, bool isNonSecuredFile = true)
        {
        ...
        private string GetFileCategory(string contentType, string fileName)
        {
        ...
        private async Task<string?> GenerateThumbnail(string category, byte[] fileBytes, string folder, string uniqueName, string contentType)
        {
            switch (category)
            {
                case "image":
                    string extension = Path.GetExtension(uniqueName);
                    string uniqueFileName = $"{Path.GetFileNameWithoutExtension(uniqueName)}_{DateTime.Now:yyyyMMddHHmmssfff}";

                    return await UploadThumbnailAsync(folder, uniqueFileName, fileBytes, contentType);

                //case "pdf":
                //    //return await GeneratePdfThumbnail(folder, fileBytes);
                //    return await UploadIcon(folder, "pdf.png");

                //case "word":
                //    return await UploadIcon(folder, "word.png");

                //case "excel":
                //    return await UploadIcon(folder, "excel.png");

                //case "ppt":
                //    return await UploadIcon(folder, "ppt.png");

                //case "zip":
                //    return await UploadIcon(folder, "zip.png");

                default:
                    return await UploadIcon(folder, "file.png");
            }
        }



        //private async Task<string?> GeneratePdfThumbnail(string folder, byte[] pdfBytes)
        //{
        ...
        //  return await UploadFileAsync(folder, "thumb.png", outputStream.ToArray());
        //}

        private async Task<string?> UploadIcon(string folder, string iconName)
        {
            string thumbPath = $"{folder}/thumbnails/{iconName}";
            return thumbPath;
        }
