...
namespace FMS_Collection.Application.Services
{
  public class AssetService
  {
    ...
    private async Task<AssetResponse> UploadDocumentToDocStore(IFormFile file, string folder, string fileName)
    {
      ...
      string originalPath = await UploadFileAsync(folder, fileName, fileBytes, contentType);

      string category = GetFileCategory(file.ContentType, file.FileName);

      string? thumbnailPath = await GenerateThumbnail(category, fileBytes, folder, originalPath, contentType);
    ...
    private async Task<string> UploadThumbnailAsync(string folder, string originalBlobPath, byte[] fileBytes, string contentType)
    {
    ...
    private async Task<string?> GenerateThumbnail(string category, byte[] fileBytes, string folder, string originalPath, string contentType)
    {
      switch (category)
      {
        case "image":
          return await UploadThumbnailAsync(folder, originalPath, fileBytes, contentType);
